...
test.describe("Scheduled publishing", () => {
  ...
  test("editing a scheduled lesson preserves the publishedAt value", async ({
    browser,
  }) => {
    const future = new Date();
    future.setDate(future.getDate() + 5);
    const futureIso = toInputDate(future);
    ...
    const teacherPage = await teacherCtx.newPage();
    await createAndPublishLesson(teacherPage, title, futureIso);
    ...
    const input = teacherPage.getByLabel(/data de publicação/i);
    await expect(input).toHaveValue(futureIso);
    await teacherCtx.close();
  });
});
