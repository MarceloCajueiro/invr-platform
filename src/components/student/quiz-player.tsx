...
export function QuizPlayer({
  ...
  if (finished) {
    if (result) {
      return (
        <div className="space-y-6">
          <ResultHeader score={result.score} feedback={result.feedback} />
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div className="h-2 bg-border rounded-full overflow-hidden">
          <div
            className="h-full bg-tarefas rounded-full transition-all duration-500"
            style={{
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    !showFeedback && !isSelected && "border-border hover:border-aulas hover:bg-aulas-bg/50",
                    !showFeedback && isSelected && "border-aulas bg-aulas-bg",
                    showCorrect && "bg-tarefas-bg border-success text-success font-medium",
                    showWrong && "bg-fora-bg border-error text-error",
                    showFeedback && !showCorrect && !showWrong && "border-border text-text-muted",
                  )}
                >
                  <span className="font-medium mr-2">{opt.letter})</span>
                  {opt.text}
          ...
          {showFeedback && (
            <div
              className={cn(
                ...
                isCorrect ? "bg-tarefas-bg text-tarefas" : "bg-fora-bg text-fora",
              )}
            >
              {isCorrect ? (
...
function ScoreBadge({ score }: { score: number }) {
  ...
  return (
    <p className={cn("text-5xl font-bold font-display", color)}>{score}%</p>
  );
}
